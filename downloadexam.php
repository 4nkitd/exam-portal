...
<?php
	...
	if($type==1){
			...
			echo '<div class="row my-3">
    			...
    			$foster1->execute();
   			    ...
   			    while($ans=$foster1->fetch(PDO::FETCH_ASSOC)){
   			 		...
   			 		$j++;
   			 		echo '<div class="col-md-6">'.$j.'). <img style="width: 30%;" src="img/ques/'.$ans['pans'].'"> </div>';
    			}
    			echo '</div>';
    			echo '<div class="row"> 
			...
			echo '<div class="row my-3">
				<div class="que col-md-12">
					<b> '.$i.'. '.base64_decode(urldecode($quest)).$brk.'<span class="devlys-hindi">'.base64_decode(urldecode($hquest)).'</span>'; 
	         if($row_pro['qimg']!="no-image") echo '<img style="width: 30%;" src="img/'.$row_pro['qimg'].'" />';
			echo '
				</div></b>';
    			...
    			$foster1->execute();
   			    ...
   			    while($ans=$foster1->fetch(PDO::FETCH_ASSOC)){
   			 	$j++;
   			 	echo '<div class="col-md-6 my-2">'.$j.'). '.base64_decode(urldecode($ans['pans'])).'<span class="devlys-hindi">'.base64_decode(urldecode($ans['hans'])).'</span>';
                if($ans["pimg"]!="no-image") echo '<img style="width: 30%;" src="img/'.$ans["pimg"].'" />';
   			 	echo '</div>';
    			}

    			$sql2 = "SELECT * FROM `gnas` WHERE `tcode`='$code' AND `qcode`='$qcode' AND `stu`='$stuid';";
    			$foster2=$con->prepare($sql2);
    			$foster2->execute();
   			    $j=0;
   			    if($ecans=$foster2->fetch(PDO::FETCH_ASSOC)){
   			 	echo '<div class="col-md-6 my-2"> Your Attempted Answer > . '.$ecans['gnas'].'</div>';
