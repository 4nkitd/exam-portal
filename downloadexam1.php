...
<?php
	...
	if($type==1){
			...
			echo '<div class="row my-3">
				<div class="que col-md-12">
					<b> '.$i.'. '.base64_decode(urldecode($quest)).''; 
	         if($row_pro['qimg']!="no-image") echo '<img style="width: 30%;" src="img/'.$row_pro['qimg'].'" />( Positive MARKS : '.$row_pro['pm'].' , Negtive MARKS : '.$row_pro['nm'].' )';
			echo '
				</div></b>';
    			...
    			$foster1->execute();
   			    ...
   			    while($ans=$foster1->fetch(PDO::FETCH_ASSOC)){
   			 	$j++;
   			 	echo '<div class="col-md-6 my-2">'.$j.'). '.base64_decode(urldecode($ans['pans']));
