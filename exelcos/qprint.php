        	...
        	while($row_pro=$foster->fetch(PDO::FETCH_ASSOC))
        	{
            	...
            	if($qimg!='no-image')
            	{
        			...
        			echo '
        				<div class="que col-md-12 pt-3">
        					<p> <b>'.$i.'. <span>'.base64_decode(urldecode($quest)).'</span>'.$brk.'<span class="devlys-hindi">'.base64_decode(urldecode($hquest)).'</span><img src="../img/'.$qimg.'"> </b></p>
        				</div>';
            			...
            			$foster1->execute();
           			    ...
           			    while($ans=$foster1->fetch(PDO::FETCH_ASSOC)){
           			 		...
           			 		$j++;
           			 		echo '<div class="col-md-6">'.$j.'). '.base64_decode(urldecode($ans['pans'])).'<span class="devlys-hindi">'.base64_decode(urldecode($ans['hans'])).'</span>'.$tmp.'</div>';
            			}
            			//echo '</div>';
            	    $i++;
        	    ...
        	    else
        	    {
        			...
        			echo '<div class="que col-md-12">
        					<p><b> '.$i.'. '.base64_decode(urldecode($quest)).$brk.'<span class="devlys-hindi">'.base64_decode(urldecode($hquest)).'</span></b></p>
        				</div><br>';
            			...
            			$foster1->execute();
          			    ...
          			    while($ans=$foster1->fetch(PDO::FETCH_ASSOC))
          			    {
                            if($ans['pimg']!="no-image") $tmp='<img src="../img/'.$ans['pimg'].'"/>';
              			 	...
              			 	$j++;
              			 	echo '<div class="col-md-6 my-1">'.$j.'). '.base64_decode(urldecode($ans['pans'])).'<span class="devlys-hindi">'.base64_decode(urldecode($ans['hans'])).'</span>'.$tmp.'</div>';
