...
function total_atms(){
	...
	try {
		...
		include("db.php");
			$sid = $_SESSION["uid"]; 
		    $vbb = "SELECT * FROM `trecord` WHERE `sid`='$sid' ;";
		    $result = $con->prepare($vbb);
		    $result->execute();
		...
		echo '<div class="card card-body py-5 ques_all_num '.$display.'" id="'.$i.'">
		        ...
		        <div id="questy">
		         <h6 class="card-header">
		         	<span class="qnum">Q. '.$i.')</span> <span class="que">'.base64_decode(urldecode($quest));
	    			...
	    			$pans[$qcode];
	   			    ...
	   			    while($ans=$foster1->fetch(PDO::FETCH_ASSOC)){
	   			 	...
	   			 	echo '<div class="col d-block">
		                    <span>'.$a[$j].'
		                     <input type="radio" id="'.$ans['pans'].'" name="'.$qcode.'" class="display-inline-block ans-'.$i.'" value="'.$j.'"> '.base64_decode(urldecode($ans['pans'])).'
		                    </span>';
